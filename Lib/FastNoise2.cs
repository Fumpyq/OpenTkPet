...
public class FastNoise2
{
    ...
    public FastNoise2(string metadataName)
    {
    ...
    private FastNoise2(IntPtr nodeHandle)
    {
    ...
    public static FastNoise2 FromEncodedNodeTree(string encodedNodeTree)
    {
        ...
        return new FastNoise2(nodeHandle);
    }
    ...
    public void Set(string memberName, FastNoise2 nodeLookup)
    {
        Metadata.Member member;
        if (!nodeMetadata[mMetadataId].members.TryGetValue(FormatLookup(memberName), out member))
        {
    ...
    //public unsafe OutputMinMax GenUniformGrid3D(float* noiseOut,
    ...
    private const string NATIVE_LIB = "FastNoise.dll";
    ...
    //[DllImport(NATIVE_LIB)]
    //private unsafe static extern uint fnGenUniformGrid3D(IntPtr nodeHandle, float* noiseOut,
    //                           int xStart, int yStart, int zStart,
    //                           int xSize, int ySize, int zSize,
    //                           float frequency, int seed, float[] outputMinMax);
