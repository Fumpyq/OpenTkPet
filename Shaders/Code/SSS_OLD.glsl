...
vec4 DrawSphere() {
    ...
    vec3 worldSpaceRayDir = (invMainCameraVP * vec4(rayDir, 0.0)).xyz;
...
vec4 simpleShadowPass(){
    ...
    vec3 mainCameraPixelWorldSpace = WorldSpaceFromDepth(dd, uv, invMainCameraVP, 0.0f);
    ...
    vec3 lightCameraPixelWorldSpace = WorldSpaceFromDepth(ldd, lightCameraUv, invLightCameraVP, 0.0f);
   ...
   // int steps2=steps*2+1;
    ...
    for(int x = -steps; x <= steps; ++x)
    {
        ...
        { 
            ...
            //mainCameraPixelWorldSpace = WorldSpaceFromDepth(dd, uv+ vec2(x, y) * texelSize2, invMainCameraVP, 0.0f);
            vec2 Luv = lightCameraUv + vec2(x, y) * texelSize;
            float ldd = texture(lightDepth, Luv).r;
            lightCameraPixelWorldSpace = WorldSpaceFromDepth(ldd, Luv , invLightCameraVP, 0.0f);
...
float penumbraScale =3.0f; // Controls the size of the soft shadow
    ...
    for (int i = -int(penumbraSize); i <= int(penumbraSize); i++) {
        for (int j = -int(penumbraSize); j <= int(penumbraSize); j++) {
            ...
            float ldd = texture(lightDepth, lUv).r;
            vec3 lightCameraPixelWorldSpace = WorldSpaceFromDepth(ldd, lUv, invLightCameraVP, 0.0f);
